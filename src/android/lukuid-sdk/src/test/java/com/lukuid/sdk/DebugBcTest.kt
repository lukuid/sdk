...
-----END CERTIFICATE-----"""
        ...
        val hash = md.digest(root.encoded).joinToString("") { "%02x".format(it) }
        println("ROOT HASH: $hash")
        
        val sig = Signature.getInstance(cert.sigAlgName, "BC")
        sig.initVerify(root.publicKey)
        sig.update(cert.tbsCertificate)
        println("VERIFY ROOT: ${sig.verify(cert.signature)}")
    }
}

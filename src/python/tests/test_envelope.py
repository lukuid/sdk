...
def test_verify_envelope_key_mismatch():
    import base64
    ...
    assert any(i.code == "ATTESTATION_FAILED" for i in issues)

def test_verify_envelope_invalid_signature():
    envelope = get_valid_envelope()
    ...
    assert any(i.code == "RECORD_SIGNATURE_INVALID" for i in issues)

def test_verify_envelope_missing_identity():
    envelope = get_valid_envelope()
    ...
    assert any(i.code == "DEVICE_IDENTITY_MISSING" for i in issues)

def test_verify_envelope_invalid_dac():
    envelope = get_valid_envelope()
...
def test_verify_envelope_invalid_canonical_string():
    envelope = get_valid_envelope()
